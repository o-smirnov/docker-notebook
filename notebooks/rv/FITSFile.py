...
class FITSFile(rv.File.FileBase):
    ...
    def __init__(self, *args, **kw):
        rv.File.FileBase.__init__(self, *args, **kw)
    ...
    def show(self,
        ...
        nrow, ncol, width = rv.File.compute_thumb_geometry(1 if unroll is None else dims[unroll], 
                                                            ncol, mincol, maxcol, width, maxwidth)
        if unroll is None:
            # show single image
            fig = make_figure and plt.figure(figsize=(width, width),dpi=rv.DPI)
            ax = plt.imshow(data[tuple(baseslice)], vmin=vmin, vmax=vmax, cmap=cmap)
            if colorbar:
                cbar = plt.colorbar()
            ...
            nrow, ncol, width = rv.File.compute_thumb_geometry(dims[unroll], ncol, mincol, maxcol, width, maxwidth)
            fig = plt.figure(figsize=(width * ncol, width * nrow), dpi=rv.DPI)
            for iplot in range(dims[unroll]):
                ...
                plt.title(title + " " + axis_labels[unroll][iplot], fontsize=fs or fs_title)
                plt.xlabel(axis_type[xyaxes[0]], fontsize=fs or fs_axis)
                plt.ylabel(axis_type[xyaxes[1]], fontsize=fs or fs_axis)
                if colorbar:
                    cbar = plt.colorbar()
